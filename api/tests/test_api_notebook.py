...
def test_create_notebook(client, token):
    ...
    assert response.data["name"] == "test_notebook_name"
    notebook = NoteBook.objects.get(pk=response.data["uuid"])
...
def test_create_note_section(client, token, test_notebook):
    response1 = client.post("/api/note_section/",
                           {
                               "notebook_uuid": test_notebook["uuid"],
    ...
    assert response1.data["name"] == "test_note_section_name"

    response2 = client.get("/api/notebook/",
                           format='json',
                           HTTP_AUTHORIZATION='Token {}'.format(token))
    assert response2.status_code
    assert response2.data["results"][0]["name"] == test_notebook["name"]
    assert len(response2.data["results"][0]["note_sections"]) == 1
    assert response2.data["results"][0]["note_sections"][0]["name"] == "test_note_section_name"
